...
```bash
grep -i "ticket" .Codex/skills/api-search/apis/INDEX.txt
```

支持中文搜索：
```bash
# 搜中文关键词
grep "创建.*部门\|部门.*创建" .Codex/skills/api-search/apis/INDEX.txt

# 搜中文操作动词
grep "分页查询" .Codex/skills/api-search/apis/INDEX.txt

# 搜中文分类
grep "部门管理" .Codex/skills/api-search/apis/INDEX.txt
```

可组合过滤：
```bash
# 只看某个 source
grep "\[staff\]" .Codex/skills/api-search/apis/INDEX.txt

# 只看 POST 接口
grep "^POST" .Codex/skills/api-search/apis/INDEX.txt

# 组合：staff 下的 ticket 相关 POST 接口
grep -i "ticket" .Codex/skills/api-search/apis/INDEX.txt | grep "\[staff\]" | grep "^POST"
```
...
cat .Codex/skills/api-search/apis/INDEX.jsonl | jq -r 'select(.path | contains("ticket")) | "\(.method) \(.path) — \(.summary)"'

# 搜索某个 tag 下的所有 API
cat .Codex/skills/api-search/apis/INDEX.jsonl | jq -r 'select(.tag == "Ticket API") | "\(.method) \(.path) — \(.summary)"'
```
